...
import com.azure.cosmos.sample.common.GSFItem;
import com.azure.cosmos.sample.common.GSFItems;
...
public class AsyncMain {
    ...
    private final String containerName = "gsf";
    ...
    private void getStartedDemo() throws Exception {
        ...
        ArrayList<GSFItem> gsfItems = new ArrayList<GSFItem>();
        for(int i=0;i<5000;i++) {
            GSFItem item = GSFItems.getItem(i);
            gsfItems.add(item);
        }
        Flux<GSFItem> gsfItemsToCreate = Flux.fromIterable(gsfItems);

        createItems(gsfItemsToCreate);

        System.out.println("Reading items.");
        readItems(gsfItemsToCreate);

        System.out.println("Querying items.");
        queryItems("SELECT * FROM gsf WHERE gsf.partitionId = '0_1' AND gsf.type = 'metadata'");
    ...
    private void createContainerIfNotExists() throws Exception {
        ...
        CosmosContainerProperties containerProperties = new CosmosContainerProperties(containerName, "/partitionId");
  ...
  private void readItems(Flux<GSFItem> gsfItemsToCreate) {
        ...
        gsfItemsToCreate.flatMap(gsfItem -> {
                            Mono<CosmosAsyncItemResponse<GSFItem>> asyncItemResponseMono = container.readItem(gsfItem.getId(), new PartitionKey(gsfItem.getPartitionId()), GSFItem.class);
    ...
    private void createItems(Flux<GSFItem> gsfItems) throws Exception {
        ...
        gsfItems.flatMap(gsfItem -> {
                return container.createItem(gsfItem);
    ...
    private void queryItems(String query) {
        ...
        CosmosPagedFlux<GSFItem> pagedFluxResponse = container.queryItems(
            query, queryOptions, GSFItem.class);
        final CountDownLatch completionLatch = new CountDownLatch(1);
